...
class AulaDocenteModelo extends connectDB
{
    ...
    public function listard($cedula)
    {
        $resultado = $this->conex->prepare("SELECT a.id as id ,a.nombre as nombre FROM usuario u, aula_docente ad, aula a WHERE u.id = ad.id_docente and ad.id_aula = a.id and u.cedula = '$cedula'");
        $respuestaArreglo = [];
        try {
            $resultado->execute();
            $respuestaArreglo = $resultado->fetchAll();
    ...
    {
        $this->id_aula = $id_aula;
        $resultado = $this->conex->prepare("SELECT  u.cedula as cedula, u.id as id, u.nombre as nombre, u.apellido as apellido, u.imagen as foto FROM aula a INNER JOIN aula_docente ae ON a.id=ae.id_aula INNER JOIN usuario u ON u.id= ae.id_docente WHERE a.id=' $this->id_aula'");
        $respuestaArreglo = [];
        try {
            $resultado->execute();
            $respuestaArreglo = $resultado->fetchAll();
